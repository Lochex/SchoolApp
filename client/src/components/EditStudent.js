...
class EditStudent extends Component {
  ...
  async componentDidMount() {
    ...
    const student = (await StudentService.show(this.props.match.params.studentId)).data;
    // console.log(this.state.students);
    this.setState({ surname: student.surname, firstname: student.firstname, regno: student.regno, email: student.email });
  }

  async onSubmit() {
    await StudentService.update(this.props.match.params.studentId, this.state);
    // console.log(this.state.students);
    // this.setState({ students: this.state.students });
  }

  render() {
    const { surname, firstname, regno, email, dob, parentno1, parentno2, gender } = this.state;
    return (
